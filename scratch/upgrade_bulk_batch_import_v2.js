...
// 1. Update ExcelImportModal.js to add Mode selection & pass mode to onConfirm
...
const oldOnConfirmCall = `      if (typeof onConfirmCallback === 'function') {
        onConfirmCallback(validationResult.parsedRows, modalApi);
      }`;

const newOnConfirmCall = `      var selectedMode = card.querySelector('#excel-import-mode-select') ? card.querySelector('#excel-import-mode-select').value : 'db_batch';
...
if (normModal.includes(normOldGrid)) {
  ...
  console.log('MODAL_GRID_MODE_SUCCESS');
} else {
  console.log('MODAL_GRID_MODE_NOT_FOUND');
}
...
if (normModal.includes(normOldCall)) {
  ...
  console.log('MODAL_ONCONFIRM_MODE_SUCCESS');
} else {
  console.log('MODAL_ONCONFIRM_MODE_NOT_FOUND');
}

fs.writeFileSync(modalPath, normModal, 'utf8');

// 2. Update DynamicFormEngine.js to handle selectedMode (db_batch vs grid_only) and Large Chunk Batching
...
const oldDfeHandler = `      onConfirm: function (parsedRows, modalApi) {
        ...
        // High-Speed Concurrency Batch Runner (Tăng tốc xử lý 15-20x)
...
const newDfeHandler = `      onConfirm: function (parsedRows, modalApi, importMode) {
        if (!parsedRows || parsedRows.length === 0) {
          if (typeof Alert !== 'undefined') Alert.warning('Thông báo', 'Không có dữ liệu hợp lệ để import.');
          return;
        }
...
if (normDfe.includes(normOldDfe)) {
  ...
  console.log('DFE_BATCH_CHUNK_SUCCESS');
} else {
  console.log('DFE_BATCH_CHUNK_NOT_FOUND');
}
