...
| `HR_TimeSheetCTReport.dat` / `WA_TimeSheetCTReport` | 2103, bật | `HR_TimeSheetDayTbl/UserAutoID` | báo cáo chi tiết | 91 field | DynamicFormEngine report | `GENERIC_CRUD_ONLY` |
...
| `WA_DonXinNghiPhepFrm` và `API_HR_NghiPhep_*` | 3030, bật | `HR_NghiPhepTbl/DocumentID`, detail/attach đã xác nhận từ schema | View/Save/Delete + chi tiết + attachment save chuyên biệt | 29 + 7 + 7 field | `APP_MODULES`, trạng thái duyệt, detail editable, đính kèm | `IMPLEMENTED` |
...
Các tên API nghỉ phép đã được xác nhận từ snapshot: `API_HR_NghiPhep`, `API_HR_NghiPhep_ChiTiet`, `API_HR_NghiPhep_Attach` và `API_HR_NghiPhep_Attach_Save`. Frontend hiện có module chuyên biệt dùng `DocumentID`, hiển thị trạng thái duyệt, cho sửa chi tiết ngày nghỉ và dùng API save attachment chuyên biệt. Không thêm action duyệt/từ chối vì snapshot chưa cung cấp contract action riêng; đây là chủ ý tránh suy đoán backend.
