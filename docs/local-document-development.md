...
Frontend on `127.0.0.1` or `localhost` calls the Document API directly at
`http://127.0.0.1:8081` and OnlyOffice at `http://127.0.0.1:8000`. Production
continues to use the `/docserver` and `/onlyoffice` reverse proxies.

Runtime overrides can be supplied before `env.js` loads:

```javascript
window.RUNTIME_CONFIG = {
    documentApiBaseUrl: 'http://127.0.0.1:8081',
    onlyOfficeBaseUrl: 'http://127.0.0.1:8000',
    useDocumentProxy: false
};
```

For Live Server on port `5500`, keep its origin in `CORS_ALLOWED_ORIGINS` in
`backend-app/.env`. Never commit the `.env` file.
