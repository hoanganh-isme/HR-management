...
| Hợp đồng | `WA_HopDongLaoDongFrm` | `HR_HopDongTbl/MaHopDong` | View, Save, Delete | Có phụ cấp, đính kèm DB và draft OnlyOffice |
...
- File chính thức dùng `HR_HopDongAttachTbl` theo `MaHopDong`.
- Danh sách mobile dùng `API_HopDongLaoDong_Attach_Metadata`, không tải `Content` hoặc `Base64Content`.
- Xem/tải một file dùng `API_HopDongLaoDong_Attach_File` theo `UserAutoID`.
- Save draft/finalize dùng `API_HopDongLaoDong_Attach_Save` để trả `UserAutoID`; không dùng `HR_Documents` trong luồng mới.
- Bản nháp OnlyOffice nằm trong `backend-app/storage/drafts`; mẫu vẫn lấy metadata từ `HR_HopDongAddfile` và file từ `backend-app/samples`.
