...
-- =========================================================================
DELETE FROM dbo.SY_FrmCfg       WHERE FID    IN ('WA_PersonQuitFrm');
DELETE FROM dbo.SY_FrmDrdwTbl   WHERE FormID IN ('WA_PersonQuitFrm');
DELETE FROM dbo.SY_FrmExpTbl    WHERE FormID IN ('WA_PersonQuitFrm');
DELETE FROM dbo.SY_FrmFltTbl    WHERE FormID IN ('WA_PersonQuitFrm');
DELETE FROM dbo.SY_FrmGrdActTbl WHERE FormID IN ('WA_PersonQuitFrm');
DELETE FROM dbo.SY_FrmMstActTbl WHERE FormID IN ('WA_PersonQuitFrm');
DELETE FROM dbo.SY_FrmOptBtnTbl WHERE FormID IN ('WA_PersonQuitFrm');
DELETE FROM dbo.SY_FrmCtrTbl    WHERE FormID IN ('WA_PersonQuitFrm');
DELETE FROM dbo.SY_FrmLstTbl    WHERE FormID = 'WA_PersonQuitFrm';
DELETE FROM dbo.SY_FormatFields  WHERE FormName = 'WA_PersonQuitFrm';
DELETE FROM dbo.WA_Menu          WHERE FormName = 'WA_PersonQuitFrm' OR MenuID = '2012';
DELETE FROM dbo.WA_API           WHERE list IN ('WA_PersonQuitFrm');
GO
...
VALUES
    (N'WA_PersonQuitFrm', N'EDIT', N'Hồ sơ nhân viên (Đã nghỉ việc)', N'Resigned Employee Profile', N'HR_PersonTbl', N'PersonID', N'Hồ sơ nhân viên (Đã nghỉ việc)');
GO

-- =========================================================================
-- 3. CẤU HÌNH LIÊN KẾT MASTER-DETAIL (SY_FrmCfg)
-- Chỉ cấu hình đúng 4 tab theo ảnh thiết kế của app desktop
-- =========================================================================
INSERT INTO dbo.SY_FrmCfg ([UserAutoID], [FID], [KeyID], [SubID], [KeyValue], [SubValue], [VDate], [PFID])
VALUES
-- Master (T0)
(NEWID(), N'WA_PersonQuitFrm', N'T0', N'TN', N'HR_PersonTbl',          N'', GETDATE(), N''),
(NEWID(), N'WA_PersonQuitFrm', N'T0', N'PK', N'PersonID',               N'', GETDATE(), N''),

-- Detail Tab 1: Quá trình làm việc và lương (HR_PersonSalaryTbl)
(NEWID(), N'WA_PersonQuitFrm', N'T1', N'TN', N'HR_PersonSalaryTbl',    N'', GETDATE(), N''),
(NEWID(), N'WA_PersonQuitFrm', N'T1', N'PK', N'UserAutoID',             N'', GETDATE(), N''),
(NEWID(), N'WA_PersonQuitFrm', N'T1', N'DCP', N'Quá trình làm việc và lương', N'', GETDATE(), N''),

-- Detail Tab 3: Khen thưởng - Kỷ luật (HR_PersonKTKLTbl)
(NEWID(), N'WA_PersonQuitFrm', N'T3', N'TN', N'HR_PersonKTKLTbl',      N'', GETDATE(), N''),
(NEWID(), N'WA_PersonQuitFrm', N'T3', N'PK', N'UserAutoID',             N'', GETDATE(), N''),
(NEWID(), N'WA_PersonQuitFrm', N'T3', N'DCP', N'Khen thưởng - Kỷ luật', N'', GETDATE(), N''),

-- Detail Tab 4: Khai báo phép năm (HR_PersonNghiPhepTbl)
(NEWID(), N'WA_PersonQuitFrm', N'T4', N'TN', N'HR_PersonNghiPhepTbl',  N'', GETDATE(), N''),
(NEWID(), N'WA_PersonQuitFrm', N'T4', N'PK', N'UserAutoID',             N'', GETDATE(), N''),
(NEWID(), N'WA_PersonQuitFrm', N'T4', N'DCP', N'Khai báo phép năm',     N'', GETDATE(), N'');
GO

-- =========================================================================
-- 4. ĐỒNG BỘ CỘT GIAO DIỆN TỪ SP API_HoSoNhanVienQuit
-- =========================================================================
EXEC [dbo].[API_DongBoTruongGiaoDien]
    @FormName   = 'WA_PersonQuitFrm',
    @ObjectName = 'API_HoSoNhanVienQuit';
GO
...
SET CaptionVN = CASE FieldName
        ...
        WHEN 'NewPersonID'        THEN N'Mã nhân viên cũ'
    ...
    FormatID = CASE
        WHEN FieldName IN ('NgaySinh', 'NgayVaoLam', 'NgayHopDong', 'NgayThuViec',
                           'SocialDate', 'NgayKetThucBH', 'ThoiGianHuongBHYT', 'NgayHetHopDong', 'NgayNghiViec', 'NgayDuKienTV', 'DateCreate', 'DateUpdate', 'CMNDNgayCap') THEN 'd'
    ...
    OrderNo = CASE FieldName
        ...
        WHEN 'GioiTinh'           THEN 41
        WHEN 'MaNVChamCong'              THEN 42
        WHEN 'PersonName2'               THEN 43
        WHEN 'PostionName'               THEN 44
        WHEN 'WorkingGroupName'          THEN 45
        WHEN 'LocationID'                THEN 46
        WHEN 'DiaChiTamTru'              THEN 47
        WHEN 'NgayNghiViec'              THEN 48
        WHEN 'DungCuLamViec'             THEN 49
        WHEN 'GhiChu'                    THEN 50
        WHEN 'NgayDuKienTV'              THEN 51
        WHEN 'isTaiTuyen'                THEN 52
        WHEN 'PersonStatusName'          THEN 15
        WHEN 'UserCreate'                THEN 54
        ...
        ELSE 99
    END
WHERE FormName = 'WA_PersonQuitFrm';
GO

-- =========================================================================
-- 6. THÊM MENU VÀO HỆ THỐNG WEB (WA_Menu) — MenuID 2012
-- =========================================================================
INSERT INTO dbo.WA_Menu (MenuID, Parent, VN, EN, FormName, FormKey, URLPara, IconClass, isDisable)
VALUES (
    '2012',
    '20',
    N'Hồ sơ nhân viên (Đã nghỉ việc)',
    'Resigned Employee Profile',
    'WA_PersonQuitFrm',
    'WA_PERSONQUITFRM',
    '#/2012',
    'badge',
...
BEGIN
    ...
    VALUES ('WA_PersonQuitFrm', 'LoaiHD', N'Loại HĐ', 'Contract Type', 'sl', 0, 'grid', 0, 0, 0, 0, 100, 1, 'API_DanhSachLoaiHD')
END
ELSE BEGIN
    UPDATE dbo.SY_FormatFields SET ShowInFilter = 1, DataSource = 'API_DanhSachLoaiHD' WHERE FormName = 'WA_PersonQuitFrm' AND FieldName = 'LoaiHD'
END
